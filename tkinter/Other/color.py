...
class BuckyButton:
  def __init__(self, master):
    ...
    #frameMain = Frame( master, bg='burlywood2' )
    frameMain = Frame( master, bg='#00ffff' )
    frameMain.grid(column=0, row=0, sticky=W+N+S+E)
    ...
    ## --- next frame ---
    ColorFrame = Frame( master, bg='blue' )
    ColorFrame.columnconfigure( 0, weight=1 )
    ColorFrame.columnconfigure( 1, weight=1 )
    ColorFrame.rowconfigure( 0, weight=1 )
    ColorFrame.rowconfigure( 1, weight=1 )
    ColorFrame.configure( bg='aquamarine' )
    ColorFrame.grid(row=1, column=0, columnspan=2, sticky=W+S+N+E )
    
    self.ColorLabel = Label( ColorFrame, bg='red', text='This is Label', fg='white' )
    self.ColorLabel.grid( sticky=W+N+E+S )
...
Mac OS X
 ...
 (note that the actual color values depend on the currently active 
 OS theme, and typically many of these will in fact be patterns 
 rather than pure colors):
