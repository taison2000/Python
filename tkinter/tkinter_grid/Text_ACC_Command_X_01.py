...
class Application(tk.Frame):
  ...
  def createWidgets(self):
    self.AccCmdText = tk.Text(self)
    self.AccCmdText.pack(side='left')
    self.AccCmdRun = tk.Button(self)
    self.AccCmdRun["text"] = "Run"
    self.AccCmdRun["command"] = self.run_raw_acc_command
    self.AccCmdRun.pack(side="top")
    
    self.QUIT = tk.Button(self, text="QUIT", fg="red", command=root.destroy)
    self.QUIT.pack(side="bottom")
    
  def run_raw_acc_command(self):
    print( self.AccCmdText.get('0.0', END) )
